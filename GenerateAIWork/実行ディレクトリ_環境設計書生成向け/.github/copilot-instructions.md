...
## Amazon Q と GitHub 連携強化ガイドライン

### Amazon Q Developer 活用パターン

プロジェクトでのAmazon Q Developer活用を最大化するため、以下のパターンに従ってください：

#### コード生成時の推奨プロンプト構造
```markdown
# Amazon Q Developer プロンプトテンプレート

## コンテキスト
- プロジェクト: JMAシステムズ AWS環境設計書生成
- 対象サービス: [具体的なAWSサービス名]
- 要件: [具体的な要件]
...
- 言語: 日本語
- 形式: Markdown
- 準拠: AWS Well-Architected Framework
- セキュリティ: 最小権限の原則適用
...
#### GitHub Actions との統合
- **自動セキュリティスキャン**: プルリクエスト時のAmazon Q セキュリティチェック
- **文書自動生成**: パラメータ更新時の設計書自動生成
- **品質検証**: AI支援による文書品質チェック

#### セキュリティ重点項目
```yaml
Amazon Q セキュリティチェック項目:
  機密情報検出:
    - ハードコードされた認証情報
    - APIキーやシークレット
    - 個人識別情報（PII）
...
#### 継続的改善プロセス
1. **週次レビュー**: Amazon Q 生成コンテンツの品質評価
2. **月次最適化**: プロンプトテンプレートの改善
3. **四半期評価**: AI統合効果の測定と改善計画策定

### GitHub Copilot との相互補完
- **Amazon Q**: AWS特化の設計・アーキテクチャ支援
- **GitHub Copilot**: 汎用的なコード生成・補完
- **連携効果**: 両ツールの強みを活かした効率的な開発

Amazon Q と GitHub の連携により、より高品質で効率的な開発プロセスを実現します。
