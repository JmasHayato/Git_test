...
本書は環境設計書における可変要素の一覧表です。system1設計書の値を規定値として、変更後の値を入力できるよう項目を追加しています。
...
3. **「変更後」が空欄の場合**: 規定値（system1設計書）の内容をそのまま環境設計書に反映する
...
| 項目名 | 規定値（system1設計書） | 変更後 | AWS参考ドキュメント | 備考 |
|--------|-------------------|--------|-------------------|------|
...
|--------|-------------------|--------|-------------------|------|
| 顧客名 | システム1 |  | - | 正式顧客名称 |
| 顧客名（英名） | system1 |  | - | システムを識別する名称 |
| プロジェクト名 | system1 |  | - | システムを識別する名称 |
| システム名 | 役割1・役割2 システム |   | - | 正式システム名称 |
| システム名（英名） | systemname1 |   | - | リソース名などで利用 |
...
| 項目名 | 規定値（system1設計書） | 変更後 | AWS参考ドキュメント | 備考 |
|--------|-------------------|--------|-------------------|------|
| ドメイン名 | system1-new-gate.com | | [Route 53](https://docs.aws.amazon.com/route53/latest/developerguide/Welcome.html) | 外部公開用ドメイン |
...
| 項目名 | 規定値（system1設計書） | 変更後 | AWS参考ドキュメント | 備考 |
|--------|-------------------|--------|-------------------|------|
| コンプライアンス要件 | systemname1社標準準拠 | | [AWS コンプライアンス](https://aws.amazon.com/compliance/) | systemname1社セキュリティポリシー準拠 |
| 定期セキュリティチェック | systemname1社側で実施 | | [AWS Security Hub](https://docs.aws.amazon.com/securityhub/latest/userguide/what-is-securityhub.html) | 社内セキュリティ監査 |
...
| アクセス制限 | systemname1社内ネットワークのみ | | [VPC セキュリティ](https://docs.aws.amazon.com/vpc/latest/userguide/security.html) | Zscaler経由でのアクセス |
...
| 個別パッチ適用 | 別途見積・発注 | | [Aurora バージョン管理](https://docs.aws.amazon.com/AmazonRDS/latest/AuroraUserGuide/Aurora.Updates.html) | system1依頼による個別対応 |
...
1. **規定値（system1設計書）**: system1設計書で定義されている現在の設定値
